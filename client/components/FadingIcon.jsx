const FadingIcon = ({ icons }) => {
  ...
  return (
    <div className="relative h-6">
      <i
        className={`${icons[0]} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xl text-violet-400`}
        style={{
          animation: `fade-in ${fadingTime}s alternate infinite ease-in-out`
        }}
      />
      <i
        className={`${icons[1]} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse text-xl text-violet-400`}
        style={{
